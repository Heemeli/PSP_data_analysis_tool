...
DENS		= True #Solar wind proton and electron densities
...
HET			= True
HETFLAG		= True #EPI-Hi HET quality flag
LET1		= False
...
if any(SWEAP) or AVEL or AMACH or NORM_BRTN or SPIFLAG or SPEFLAG or SPCFLAG:
	...
	heliodist_x		= SPI['psp_spi_SUN_DIST']['x']
	heliodist_y		= SPI['psp_spi_SUN_DIST']['y']
	PED_x			= SPI['psp_spi_EFLUX_VS_ENERGY']['x']
	PED_y			= SPI['psp_spi_EFLUX_VS_ENERGY']['y']
	PED_v			= SPI['psp_spi_EFLUX_VS_ENERGY']['v'][0]
	...
	if not(SPI_OPT):
		...
		VRTN_x		= SPC['psp_spc_vp_moment_RTN']['x']
		VRTN_y		= SPC['psp_spc_vp_moment_RTN']['y']
		protdens_x	= SPC['psp_spc_np_moment']['x']
		protdens_y	= SPC['psp_spc_np_moment']['y']
		spcflag_x	= SPC['psp_spc_general_flag']['x']
		spcflag_y	= SPC['psp_spc_general_flag']['y']
		Vlabel		= 'SPC'
	else:
		VRTN_x		= SPI['psp_spi_VEL_RTN_SUN']['x']
		VRTN_y		= SPI['psp_spi_VEL_RTN_SUN']['y']
		protdens_x	= SPI['psp_spi_DENS']['x']
		protdens_y	= SPI['psp_spi_DENS']['y']
		Vlabel		= 'SPI'
...
if any(FIELDS) or AVEL or AMACH or FIELDSFLAG:
	...
	BRTN_x		= FIELDSdict['psp_fld_l2_mag_RTN_4_Sa_per_Cyc']['x']
	BRTN_y		= FIELDSdict['psp_fld_l2_mag_RTN_4_Sa_per_Cyc']['y']
	elecdens_x	= FIELDSdict[f'{QTN}']['x']
	elecdens_y	= FIELDSdict[f'{QTN}']['y']
...
if any(ISOIS) or HETFLAG or LET1FLAG or EPILOFLAG:
	...
	if EPILO:
		...
		if EPILOFLAG:
			...
			chanPflag_y	= EPILOd['psp_epihi_Quality_Flag_ChanP']['y']
			chanCflag_x	= EPILOd['psp_epilo_Quality_Flag_ChanC']['x']
			chanCflag_y	= EPILOd['psp_epihi_Quality_Flag_ChanC']['y']
			chanDflag_x	= EPILOd['psp_epilo_Quality_Flag_ChanD']['x']
			chanDflag_y	= EPILOd['psp_epihi_Quality_Flag_ChanD']['y']
			chanEflag_x	= EPILOd['psp_epilo_Quality_Flag_ChanE']['x']
			chanEflag_y	= EPILOd['psp_epihi_Quality_Flag_ChanE']['y']
		for i in ISOISvars:
			if i[0]:
				if i[1][2] == None:
					continue
...
if NORM_BRTN or MAGB or AVEL or AMACH:
	#Absolute B
	absB			= np.linalg.norm(BRTN_y, axis=1)
	if NORM_BRTN or AVEL or AMACH:
		...
		SPIepoch		= np.float64(heliodist_x)
		FIELDSepoch		= np.float64(BRTN_x)
		f1 = scipy.interpolate.interp1d(SPIepoch, heliodist_y, fill_value='extrapolate')
...
if AVEL or AMACH:
	...
	if SPI_OPT:
		f2p = scipy.interpolate.interp1d(SPIepoch, protdens_y, fill_value='extrapolate')
	else:
		SPCepoch = np.float64(protdens_x)
		f2p = scipy.interpolate.interp1d(SPCepoch, protdens_y, fill_value='extrapolate')
	...
	if ELECDENS:
		FIELDSQTNepoch = np.float64(elecdens_x)
		f2e = scipy.interpolate.interp1d(FIELDSQTNepoch, elecdens_y, fill_value='extrapolate')
	...
	if AMACH:
		...
		if SPI_OPT:
			f4 = scipy.interpolate.interp1d(SPIepoch, VRTN_y[:,0], fill_value='extrapolate')
		else:
			f4 = scipy.interpolate.interp1d(SPCepoch, VRTN_y[:,0], fill_value='extrapolate')
...
if EED: specgram_plot(EED_x, EED_v, EED_y, 'Energy [eV]', 'Electron', intensity=True)
if PED: specgram_plot(PED_x, PED_v, PED_y, 'Energy [eV]', 'Proton', intensity=True)
...
if HELIODIST:
	...
	dist.set_xticks(heliodist_x[::4000])
	dist.set_xticklabels(np.round(heliodist_y[::4000]/696340, 1))
